...
const PipelineFunnel = () => {
  const max = Math.max(...pipelineFunnel.map((s) => s.count));

  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-lg">Hiring pipeline</CardTitle>
        <CardDescription>Candidate progression across stages</CardDescription>
      </CardHeader>
      <CardContent className="space-y-3">
        {pipelineFunnel.map((stage, i) => {
          const pct = (stage.count / max) * 100;
          const conversion =
            i === 0 ? null : Math.round((stage.count / pipelineFunnel[i - 1].count) * 100);
          return (
